...
DATA_ROOT_DIR/
    └──features/
        ...
        └── ...
```
DATA_ROOT_DIR is the base directory of all datasets (e.g. the directory to your SSD) and features is a folder containing features from each slide stored as a .pt files. Please refer to refer to [CLAM](https://github.com/mahmoodlab/CLAM) for examples on tissue segmentation and featue extraction. 
...
```
where p_val and p_test control the percentage of samples should be used for validation and testing, respectivelly. Similar approach is used to split the data for the Grading network.
